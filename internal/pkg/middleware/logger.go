	...
	return func(c *gin.Context) {
		...
		if c.Request.Method == http.MethodPost || c.Request.Method == http.MethodPut {
			...
			if mediaType == "application/json" {
				if v, ok := c.Get(common.ReqBodyKey); ok {
		...
		if v, ok := c.Get(common.ResBodyKey); ok {
		...
		ctx = log.NewTag(ctx, log.TagKeyRequest)
		log.Context(ctx).Info(fmt.Sprintf("[HTTP] %s-%s-%d (%dms)",
